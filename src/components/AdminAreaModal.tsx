...
export default function AdminAreaModal({ isOpen, onClose, lang = 'pt' }: AdminAreaModalProps) {
  ...
  const [superadminEmail, setSuperadminEmail] = useState<string>('helpus.ecommerce@gmail.com');
  ...
  useEffect(() => {
    const authStored = localStorage.getItem('helpus_admin_authenticated');
    if (authStored === 'true') {
  ...
  // Handle Google Login simulation for Superadmin
  const handleGoogleLogin = (emailChoice?: string) => {
    if (!captchaVerified) {
      alert('Por favor, complete a verificação de segurança (CAPTCHA) antes de entrar.');
      return;
    }
    const targetEmail = emailChoice || superadminEmail;
    setSuperadminEmail(targetEmail);
    localStorage.setItem('helpus_admin_authenticated', 'true');
    localStorage.setItem('helpus_admin_email', targetEmail);
    setIsAuthenticated(true);
  };

  // Handle Logout
  const handleLogout = () => {
    localStorage.removeItem('helpus_admin_authenticated');
    localStorage.removeItem('helpus_admin_email');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-slate-950/80 backdrop-blur-md animate-fade-in overflow-y-auto">
      <div className="bg-slate-900 border border-slate-700/80 rounded-2xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden my-auto text-slate-100">
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-6">
          ...
          {!isAuthenticated ? (
            /* ================= STATE 1: UNAUTHENTICATED & EXPLANATORY LOGIN SCREEN ================= */
            <div className="space-y-6 max-w-3xl mx-auto py-2">
              ...
              <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 text-center space-y-4 shadow-inner">
                ...
                <h4 className="text-xl font-bold text-white">
                  Bem-vindo à Área Administrativa do HelpUS Accounting
                </h4>
                <p className="text-sm text-slate-300 leading-relaxed max-w-2xl mx-auto">
                  Esta área restrita destina-se ao acompanhamento quantitativo e telemetria de uso da plataforma.
                  Através deste painel, administradores autorizados acompanham os volumes de processamento de NFS-e, NF-e, CT-e, EFD-Reinf, SPED e OFX em tempo real para prover melhorias contínuas e evitar erros.
                </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-left pt-2">
                  ...
                  <div className="bg-slate-900 border border-slate-800 p-3.5 rounded-xl space-y-1.5">
                    <div className="flex items-center gap-2 text-amber-400 font-bold text-xs">
                      <Users className="w-4 h-4" /> Auditoria & Usuários
                    </div>
                    <p className="text-xs text-slate-400">
              ...
              <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 space-y-5">
                <div className="border-b border-slate-800 pb-3 flex items-center justify-between">
                  <h5 className="font-bold text-white text-sm flex items-center gap-2">
                    <UserCheck className="w-4 h-4 text-amber-400" /> Autenticação de Superadmin
                  </h5>
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-slate-400">
                        {captchaVerified
                          ...
                          : 'Clique na caixa para validar a verificação CAPTCHA'}
                      </span>
                    </div>
                  </div>

                  {/* HelpUS Shield Logo Mini */}
                ...
                <div className="space-y-3 pt-1">
                  <button
                    onClick={() => handleGoogleLogin('helpus.ecommerce@gmail.com')}
                    className={`w-full py-3 px-4 rounded-xl font-bold text-sm flex items-center justify-center gap-3 border transition cursor-pointer shadow-lg ${
                      captchaVerified
                        ? 'bg-slate-900 hover:bg-slate-850 text-white border-slate-700 hover:border-amber-500/50 hover:shadow-amber-500/10'
                  ...
                  >
                    ...
                    <span>Entrar com Google (helpus.ecommerce@gmail.com)</span>
                  </button>

                  <div className="flex items-center justify-between text-[11px] text-slate-400 px-1 pt-1">
                    <span>Função atribuída: <strong>Superadmin Corporate</strong></span>
                    <span>Modo: <strong>Produção (Live)</strong></span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-xs text-slate-400">
                      Sessão ativa de monitoramento e auditoria quantitativa
                    </span>
                  </div>
                </div>
